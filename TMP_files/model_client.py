...
class InternalGatewayChatModel(BaseChatModel):
    ...
    def _generate(self, messages: List[BaseMessage], stop=None, run_manager=None, **kwargs) -> ChatResult:
        ...
        dprint("FLATTENED CONTENTS (this is what the model actually sees as history)",
        ...
        from tools.debug_utils import dprint
        dprint("GATEWAY REQUEST", json.dumps(body, indent=2)[:3000])

        response = requests.post(self.endpoint_url, headers=headers, json=body, timeout=120)
        response.raise_for_status()
        data = response.json()

        dprint("GATEWAY RAW RESPONSE", json.dumps(data, indent=2)[:3000])
        ...
        dprint("PARSED AIMESSAGE",
               f"tool_calls={ai_message.tool_calls}",
               f"content={ai_message.content[:500]}")
