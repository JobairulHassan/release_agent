...
def route_to_version_resolve(state):
    ...
    dprint("PHASE 1 DISPATCH", f"changed_repos={state['changed_repos']}")
...
def version_resolve_node(task, config):
    ...
    try:
        ...
        # Full transcript -- shows every message including ToolMessages, so
        # you can see whether the model actually received tool results.
        dprint(f"FULL TRANSCRIPT for {repo_name}", *[
            f"[{m.__class__.__name__}] {str(m.content)[:400]}"
            + (f" tool_calls={m.tool_calls}" if getattr(m, "tool_calls", None) else "")
            for m in result["messages"]
        ])

        final_text = result["messages"][-1].content
        dprint(f"FINAL TEXT for {repo_name}", final_text)

        match = re.search(r"VERSION=(\S+)", final_text)
        new_version = match.group(1) if match else "UNRESOLVED"
        dprint(f"PARSED VERSION for {repo_name}", new_version)
